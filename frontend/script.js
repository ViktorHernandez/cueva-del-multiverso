document.addEventListener("DOMContentLoaded", () => {
    ...
    async function loadCart() {
        ...
        try {
            const res = await fetch(`${API_BASE}/cart/${currentUser.email}`);
            if (res.ok) {
                cart = await res.json();
            }
        } catch (err) {
    ...
    async function saveCart() {
        if (!currentUser) return;
        try {
            await fetch(`${API_BASE}/cart/${currentUser.email}`, {
                method: "PUT",
                headers: { "Content-Type": "application/json" },
    ...
    profileForm.addEventListener("submit", async (e) => {
        ...
        try {
            const payload = { name: newName, email: newEmail, phone: newPhone };
            if (newPassword) payload.password = newPassword;

            const res = await fetch(`${API_BASE}/users/${currentUser.email}`, {
                method: "PUT",
                headers: { "Content-Type": "application/json" },
            ...
            sessionStorage.setItem("sessionEmail", currentUser.email);
            if (newPassword) sessionStorage.setItem("sessionPassword", newPassword);
    ...
    if (downloadInvoiceBtn) {
        downloadInvoiceBtn.addEventListener("click", async () => {
            if (!lastOrder && currentUser) {
                try {
                    const res = await fetch(`${API_BASE}/orders/last/${currentUser.email}`);
                    if (res.ok) {
    ...
    window.deleteUser = async function(email) {
        ...
        try {
            const res = await fetch(`${API_BASE}/users/${email}`, { method: "DELETE" });
            if (!res.ok) {
